    ...
    def __init__(self, wm, act_space, config):
        ...
        for key, scale in config.prior_rewards.items():
            ...
            if key == "extr":
                ...
                self.rewards[key] = rewfn
        scales = {k: v for k, v in config.prior_rewards.items() if v}
        self.ac = agent.MyImagActorCritic(critics, scales, act_space, config,wm.inverse_model, name="ac")
    ...
    def __init__(self, wm, act_space, config):
        ...
        for key, scale in config.prior_rewards.items():
            ...
            if key == "extr":
                ...
                self.rewards[key] = rewfn
        scales = {k: v for k, v in config.prior_rewards.items() if v}
        self.ac = agent.MyImagActorCritic2(critics, scales, act_space, config,wm.action_predictor, name="ac")
