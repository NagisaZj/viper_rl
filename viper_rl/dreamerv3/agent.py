...
class MyAgent(nj.Module):
    ...
    def train(self, data, state, reference_data=None):
        ...
        state, wm_outs, mets = self.wm.train(data, state, reference_data)
        metrics.update(mets)
        context = {**data, **wm_outs["post"],**wm_outs}
        start = tree_map(lambda x: x.reshape([-1] + list(x.shape[2:])), context)
        _, mets = self.task_behavior.train(self.wm.imagine, start, context)
        metrics.update(mets)
        if self.config.expl_behavior != "None":
...
class MyWorldModel(nj.Module):
    def __init__(self, obs_space, act_space, config,ae):
        ...
        self.inverse_model = nets.MLP(self.act_space.shape, **config.inverse_model, name="inverse_model")
    ...
    def train(self, data, state, reference_data=None):
        modules = [self.encoder, self.rssm, *self.heads.values(),self.embedding_encoder,self.inverse_model]
        if self.amp:
            modules.append(self.discriminator)
        mets, (state, outs, metrics) = self.opt(
            modules, self.loss, data, state, reference_data, has_aux=True
    ...
    def loss(self, data, state, reference_data=None):
        ...
        rep_input={'representation':sg(representation_encoding.reshape(*representation_encoding.shape[:2],-1))}
        ...
        action_label = sg(jnp.concatenate([prev_action[:,None,...], data['action'][:,:-1,...]], 1)).astype(jnp.float32)
        losses['inverse_loss'] =-inverse_prediction.log_prob(action_label.astype(jnp.float32))
        assert losses['inverse_loss'].shape == embed.shape[:2], ('inverse_loss', losses['inverse_loss'].shape)
        
        pred_input={'representation':sg(prediction_encoding.reshape(*prediction_encoding.shape[:2],-1))}
        ...
        action_bc_loss = sg(inverse_prediction_prediction.log_prob(action_label))
        ...
        for name, head in self.heads.items():
            if name=='bc_reward':
                # a=1
                out = head({'deter':pd,'stoch':ps,'action':action_label} if name in self.config.grad_heads else sg({'deter':pd,'stoch':ps,'action':action_label}))
                out = out if isinstance(out, dict) else {name: out}
                dists.update(out)
            else:
                if name == "discriminator_reward":
        ...
        out = {"embed": embed, "post": post, "prior": prior}
        out.update({f"{k}_loss": v for k, v in losses.items()})
        out.update({'inverse_action':sg(inverse_prediction_prediction.mode()),'prev_stoch':ps,'prev_deter':pd})
        last_latent = {k: v[:, -1] for k, v in post.items()}
        last_action = data["action"][:, -1]
        state = last_latent, last_action
        metrics = self._metrics(data, dists, post, prior, losses, model_loss)
...
class MyImagActorCritic(nj.Module):
    ...
    def train(self, imagine, start, context):
        def loss(start,context):
            policy = lambda s: self.actor(sg(s)).sample(seed=nj.rng())
            traj = imagine(policy, start, self.config.imag_horizon)
            loss, metrics = self.loss(traj,context)
            return loss, (traj, metrics)

        mets, (traj, metrics) = self.opt(self.actor, loss, start,context, has_aux=True)
        metrics.update(mets)
        for key, critic in self.critics.items():
            if key != 'bc_reward':
                mets = critic.train(traj, self.actor)
    ...
    def loss(self, traj,context):
        ...
        inputs={'deter':context['prev_deter'],'stoch':context['prev_stoch']}
        new_policy = self.actor(sg(inputs))
        imitation_logpi = new_policy.log_prob(sg(context["inverse_action"]))
        metrics.update(jaxutils.tensorstats(imitation_logpi, "imitation_logpi"))
        loss-=imitation_logpi.mean()*self.config.loss_scales.inverse_action
        loss *= self.config.loss_scales.actor
        metrics.update(self._metrics(traj, policy, logpi, ent, adv))
        return loss.mean(), metrics
