...
def load_dataset_long(config, train, modality='video'):
    num_data_local = jax.local_device_count()
    num_ds_shards = jax.process_count()
    ds_shard_id = jax.process_index()
...
def load_npz(config, data_path, train, n_shards, shard_id, mask):
    ...
    def process(video):
        if hasattr(config, 'seq_len'):
            ...
            length = tf.cast( tf.floor(max_idx / 4),tf.int32)
            random_choice = tf.random.uniform((), 0, 100, dtype=tf.int32)/100
            if random_choice<=0.8:
                idx = tf.random.uniform((), 0, length, dtype=tf.int32)
            elif random_choice<=0.9:
                idx = tf.random.uniform((), length, length*2, dtype=tf.int32)
            elif random_choice<=0.95:
                idx = tf.random.uniform((), length*2, length*3, dtype=tf.int32)
            else:
                idx = tf.random.uniform((), length*3, length*4, dtype=tf.int32)
            video = video[idx:]
...
def load_npz_long(config, data_path, train, n_shards, shard_id, mask):
    split = 'train' if train else 'test'
    folder = osp.join(data_path, split, '*.npz')
    fns = _glob_files(folder)
    random.Random(1234).shuffle(fns) 
    ...
    def process(video):
        if hasattr(config, 'seq_len'):
            ...
            idx = tf.random.uniform((), 0, max_idx, dtype=tf.int32)
            # print(video.shape)
            new_video = tf.concat([video]+[video[-1:]]*800,0)
            new_video_plus_100 = new_video[idx+100:-700]
            new_video_plus_200 = new_video[idx+200:-600]
            new_video_plus_400 = new_video[idx+400:-400]
            new_video_plus_800 = new_video[idx+800:]
            video = video[idx:]
            ...
            new_video_plus_100 = new_video_plus_100[:tf.shape(new_video_plus_100)[0] // req_len * req_len]
            new_video_plus_100 = tf.reshape(
                new_video_plus_100,
                tf.concat(([tf.shape(new_video_plus_100)[0] // req_len, req_len], tf.shape(new_video_plus_100)[1:]), 0)
            )
            new_video_plus_100 = new_video_plus_100[:, ::config.frame_skip]
            
            new_video_plus_200 = new_video_plus_200[:tf.shape(new_video_plus_200)[0] // req_len * req_len]
            new_video_plus_200 = tf.reshape(
                new_video_plus_200,
                tf.concat(([tf.shape(new_video_plus_200)[0] // req_len, req_len], tf.shape(new_video_plus_200)[1:]), 0)
            )
            new_video_plus_200 = new_video_plus_200[:, ::config.frame_skip]
            
            new_video_plus_400 = new_video_plus_400[:tf.shape(new_video_plus_400)[0] // req_len * req_len]
            new_video_plus_400 = tf.reshape(
                new_video_plus_400,
                tf.concat(([tf.shape(new_video_plus_400)[0] // req_len, req_len], tf.shape(new_video_plus_400)[1:]), 0)
            )
            new_video_plus_400 = new_video_plus_400[:, ::config.frame_skip]
            
            new_video_plus_800 = new_video_plus_800[:tf.shape(new_video_plus_800)[0] // req_len * req_len]
            new_video_plus_800 = tf.reshape(
                new_video_plus_800,
                tf.concat(([tf.shape(new_video_plus_800)[0] // req_len, req_len], tf.shape(new_video_plus_800)[1:]), 0)
            )
            new_video_plus_800 = new_video_plus_800[:, ::config.frame_skip]
            
            video = tf.concat([video,new_video_plus_100,new_video_plus_200,new_video_plus_400,new_video_plus_800],1)
